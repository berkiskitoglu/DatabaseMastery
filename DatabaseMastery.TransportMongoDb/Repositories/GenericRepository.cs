...
namespace DatabaseMastery.TransportMongoDb.Repositories
{
    public class GenericRepository<T> : IGenericRepository<T> where T : IEntity
    {
        ...
        public async Task<List<TField>> GetDistinctAsync<TField>(string field, FilterDefinition<T> filter)
        {
            var cursor = await _collection.DistinctAsync<TField>(field, filter);
        ...
        public async Task UpdateByFilterAsync(FilterDefinition<T> filter, UpdateDefinition<T> update)
        {
